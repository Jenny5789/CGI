...
if use_fp16:
    # UNet + ControlNet already declare use_fp16 in anydoor.yaml (internal
    # x.type(self.dtype) casting), so it's safe to store their weights in fp16.
    # DINOv2 (cond_stage) and the VAE (first_stage) stay fp32 for stability.
    model.model = model.model.half()
    model.control_model = model.control_model.half()
    # GroupNorm32 explicitly upcasts its input to fp32 for numerical stability
    # (see ldm/modules/diffusionmodules/util.py), so its own weight/bias must
    # stay fp32 too or the internal x.float() call breaks against fp16 params.
    for m in list(model.model.modules()) + list(model.control_model.modules()):
